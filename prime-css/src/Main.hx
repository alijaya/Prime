...
class Main
{
	...
	{
		  new mcli.Dispatch(Sys.args() ).dispatch(new StyleBuilder());
	}
}

class StyleBuilder extends mcli.CommandLine
{
    ...
    public function buildstyles(projectDir:String,?args:String)
    {
        ...
        parserSources.push('build-cssparser.hxml');
        parserSources.push('prime//tools//CSSParserMain.hx');
        parserSources.push('prime//tools//CSSParser.hx');
        
        var parserBin = 'parser.js';
        var buildArgs = 'haxe build-cssparser.hxml -main prime.tools.CSSParserMain -js $parserBin';
        ...
        if (compileStyles || !FileSystem.exists('$projectDir//styles//StyleSheet.hx') || !genedFileNewerThan('$projectDir//styles//StyleSheet.hx', stylesSources))
        {
            ...
            var compileStylesArgs = 'node parser.js "$projectDir//styles" $primeCSSPath';
            Sys.command(compileStylesArgs);
        }
        else
        {
    ...
    private function genedFileNewerThan(generatedFile:String, sourceFiles:Array<String>)
    {
